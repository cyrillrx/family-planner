...
Nothing yet. `cmp-app/` is not initialized, so this project has no concrete bindings to document.

The PR that initializes the KMP project adds them here or in [`AGENTS.md`](../../AGENTS.md): module split, package layout, dependency injection, navigation pattern, design system tokens, test location and coverage policy.
